...
const UpdateItem = () => {
  ...
  useEffect(() => {
    const url = `https://pencil-book-server.vercel.app/inventory/${id}`;
    fetch(url, {
      method: "GET",
      headers: {
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const { data } = await axios.put(
        `https://pencil-book-server.vercel.app/inventory/${id}`,
        setupdateItem,
        {
          headers: {
            "content-type": "application/json",
  ...
  const handleDelivery = async (event) => {
    ...
    try {
      if (
        ...
        const { data } = await axios.put(
          `https://pencil-book-server.vercel.app/delivery/${id}`,
          deliveryItem,
          {
            headers: {
              "content-type": "application/json",
